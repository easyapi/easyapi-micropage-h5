      ...
      <view v-if="component.template === '轮播海报'" class="ad-po">
        <swiper style="height:400rpx" class="ad-swiper" circular="true" indicator-color="#ddd"
          ...
          <swiper-item v-for="item in component.items">
            <image :style="{borderRadius:component.borderRadius,height:loadFlag?'1px':imageHeight}" :src="item.img"
              class="ad-swiper-item" mode="widthFix" lazy-load="true" @load="bindload">
            </image>
          </swiper-item>
        </swiper>
      </view>
